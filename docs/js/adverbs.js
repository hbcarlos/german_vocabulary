...
async function fetchData() {
  try {
    const response = await fetch('./data/adverbs.json');
    if (!response.ok) {
      throw new Error('Failed to load adverbs data.');
    }
    return response.json();
  } catch (error) {
    console.error(error);
    verbDisplay.textContent = 'Error';
    resultsContainer.textContent = 'No se pudieron cargar los adverbios.';
  }
  return []
}
