...
async function fetchData() {
  try {
    ...
    if (!response.ok) {
      throw new Error('Failed to load adjectives data.');
    }
    return response.json();
  } catch (error) {
    console.error(error);
    verbDisplay.textContent = 'Error';
    resultsContainer.textContent = 'No se pudieron cargar los adjetivos.';
  }
  return []
}
